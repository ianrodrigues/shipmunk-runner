...
const reviewIntentInstructions = "The task context may include an <author_intent> block holding the pull request's " +
	...
	"defect. State explicitly whether the declared intent and the observed behavior align. Review only the files " +
	"in the planned changed-file list above; the task context's <changed_files> block restates that same list " +
