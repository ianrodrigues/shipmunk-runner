...
The runner persists claim state before artifact preparation and records a created sandbox before starting it. It heartbeats immediately after claim and on a timed 10-second cadence through preparation and execution against a 45-second lease. An independent `shipmunk-watchdog` process, launched as its own binary rather than a forked child, tracks renewals with a monotonic timer. Parent death, heartbeat loss, or deadline expiry stops and removes the whole container process tree with TERM and KILL escalation while leaving durable state for restart acknowledgement. Durable state is cleared only after the container and workspace are confirmed absent and the server accepts a fenced `stopped` acknowledgement. Cleanup or acknowledgement failure retains that state, so restart reconciliation must retry it before asking for replacement work. The `stopped` acknowledgement is refused only for a fence mismatch or a non-current attempt, both permanent, so the journal counts refusals of that kind (a differently failed acknowledgement neither advances nor resets the count) and treats a third as settled, clearing state without a further retry; `shipmunk-runner run --discard-attempt` clears it immediately instead, for an operator who does not want to wait on that bound. Neither path notifies the application of a confirmed stopped sandbox for that attempt, since the refusal itself is what proves one is unreachable, so the run's capacity reservation is not released by the runner; it stays held until the application's own bookkeeping reclaims it (ianrodrigues/shipmunk-tasks#103 tracks a server-side sweep for that).
...
Native clients can explicitly create metadata with broader permissions than the runner's umask. After the native container has stopped, the runner validates the entire owned home for unsafe entries, then reduces regular files to `0600` and subdirectories to `0700` while holding the profile lock. The enclosing home stays `0700` throughout. Links, special files, multiple hard links, foreign ownership and special permission bits are rejected before any permissions change; an unsafe entry keeps the execution reservation quarantined rather than releasing it. `.codex/tmp` is the native runtime's own scratch directory: the runner replaces that directory, contents and all, inside the home and without following links, before it inspects anything, so neither a helper symlink the runtime left behind nor a mount point Docker created can quarantine the profile. The pre-execution and recovery checks normalize the home before they validate it, so metadata the native client wrote with broader permissions is repaired rather than reported as unsafe; every path outside the scratch directory stays strict. Foreign-owner coverage runs on Linux, where bind mounts preserve ownership.
...
The token needs `runner:profiles` and belongs to the assigned runner. The root and token must belong to the dedicated, non-root runner account; directory permissions are 0700 and token permissions 0600. Do not use a desktop credential directory. Login requires an interactive terminal: native URLs/codes go directly there, never through application events. Codex uses `login --device-auth`; Claude uses `auth login --claudeai`. No personal credentials are discovered or copied. `connect` prints a sentence such as "Runner is ready." or "Runner is not ready: <reason>."; pass `--json` for the machine-readable health object. A failed `--json` operation still emits `{"health":"error","reason":"operation_failed"}` on stdout, with exit `1`.

`shipmunk-runner probe` (same flags) rechecks the existing profile without logging in again, and accepts `--json` without a terminal for scripts; `shipmunk-runner disconnect` (same flags) removes local access after the application requests disconnection. None of connect, probe or disconnect accept a caller-supplied `--operation`; each subcommand sets it. Each operation has a 45-second renewable control-plane lease, a 15-minute overall bound and independent process-tree watchdog. Restart by repeating the command with the original operation ID to reconcile interrupted cleanup/acknowledgement; after reconciliation, use a fresh ID for new work. Definitive rejected begins do not leave a local journal. Ambiguous responses retain recovery state and block replacement operations.
