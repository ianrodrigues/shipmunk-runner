...
`make go-build VERSION=v1.2.3` builds the two operational Go commands with that exact version embedded. An ordinary local build reports `development`; release packaging must always supply its validated release tag. Each resulting `shipmunk-runner` and `shipmunk-watchdog` binary reports the embedded value with `--version`.
